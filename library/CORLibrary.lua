...
function library.new()
    ...
    self.precast = function(spell)
        ...
        if not user_midaction() and sets then
            ...
            elseif spell.name == 'Ranged' then
                ...
                if settings['Flurry'] then
                    print(string.format('%s: Flurry', spell.name))
    ...
    self.midcast = function(spell)
        ...
        if sets then
            ...
            if user and user.Midcast(bp, spell) then
                return
            end
            
            if spell.name == 'Ranged' then
                local type = settings['Ranged Type']

                if tpye == 2 then

                    if T(bp.res.buffs):contains(433) then
                        equip(set_combine(sets['Ammo'], sets['Ranged'][settings['Ranged Types'][type]][modes.combat][modes.ranged].set, {body='Arc. Jerkin +3'}))
                            
                    elseif T(bp.res.buffs):contains(467) then
                        equip(set_combine(sets['Ammo'], sets['Ranged'][settings['Ranged Types'][type]][modes.combat][modes.ranged].set, {hands='Lanun Gants +3'}))

                    end

                else
                    equip(set_combine(sets['Ammo'], sets['Ranged'][settings['Ranged Types'][type]][modes.combat][modes.ranged].set))
    ...
    self.aftercast = function(spell)
        ...
        if sets then
            ...
            if user and user.Aftercast and user.Aftercast(bp, spell) then
                return
            end
